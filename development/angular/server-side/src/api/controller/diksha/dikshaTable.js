...
router.get('/dikshaMetaData', auth.authController, async (req, res) => {
    try {
        ...
        var tableData = await s3File.storageType == "s3" ? await s3File.readS3File(fileName) : await s3File.readLocalFile("/schoolData.json");;
...
router.post('/dikshaTimeRangeTableData', auth.authController, async (req, res) => {
    try {
        ...
        if (req.body.collectionType && distId && timePeriod) {
            ...
            tableData = await s3File.storageType == "s3" ? await s3File.readS3File(fileName) : await s3File.readLocalFile("/schoolData.json");;
