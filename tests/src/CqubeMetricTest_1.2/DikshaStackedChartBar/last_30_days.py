...
class DikshaStackedReport(unittest.TestCase):
    ...
    def test_metrics(self):
        print("")
        last_30_days = DikshaStacked()
        df1 = pd.read_sql_query(last_30_days.last_30_day, self.connection)
       ...
       'content_subject', 'total_content_plays']]
        ...
        path = config['jsondata']['stack_bar_reports_last_30_days']
        files = glob.glob(path)
        list = []
        for f1 in files:
            with open(f1, "r") as file:
                if os.path.basename(f1) == 'All.json':
                    continue
