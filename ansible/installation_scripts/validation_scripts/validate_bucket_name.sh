...
if [ $? == 0 ]
then
    tput setaf 1; echo "Error: [ $2 ] Bucket Owned or exists. Please change the bucket name"; tput sgr0
    count=1
...
then
    tput setaf 1; echo "Error: [ $2 ] Bucket exists but not owned. Please change the bucket name"; tput sgr0
    count=1
elif [[ "$bucketstatus" == *"Bad Request"* ]]
then
  tput setaf 1; echo "Error: [ $2 ] Bucket name should be between 3 and 63 characters. Please change the bucket name"; tput sgr0
  count=1
else
  tput setaf 1; echo "Error: [ $2 ] $bucketstatus"; tput sgr0
  count=1
fi
return "$count"
}
