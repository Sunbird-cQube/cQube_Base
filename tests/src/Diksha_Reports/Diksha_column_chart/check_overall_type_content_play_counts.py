...
class test_overall_types_data():
    ...
    def test_last30_days(self):
        ...
        self.data.page_loading(self.driver)
        coll_type = Select(self.driver.find_element_by_id('coll_type'))
        coll_type.select_by_index(1)
        self.driver.find_element_by_id('download').click()
        time.sleep(3)
        self.filename = self.p.get_download_dir() + "/collectionType_all_data.csv"
        if not os.path.isfile(self.filename):
            print("Diksha Overall type of  last 30 days data csv file not downloaded")
        else:
            with open(self.filename) as fin:
                ...
                if int(pc) != int(contentplays):
                    print("Overall type of last 30 days has difference between screen count value and csv file count ")
                    count = count + 1
            self.data.page_loading(self.driver)
    ...
    def test_last_day(self):
        ...
        if not os.path.isfile(self.filename):
            ...
            with open(self.filename) as fin:
                ...
                pc = re.sub('\D', "", play_count)
                if int(pc) != int(contentplays):
                    print("over all type of last 30 days has difference between screen count value and csv file count ")
                    count = count + 1
            self.data.page_loading(self.driver)
        os.remove(self.filename)
        return count
