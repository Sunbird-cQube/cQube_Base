...
sudo az storage blob upload-batch --connection-string "DefaultEndpointsProtocol=https;AccountName=$azure_account_name;AccountKey=$azure_account_key;EndpointSuffix=core.windows.net" --destination $azure_input_container --source $source_input_directory > /dev/null 2>&1
fi

if [[ $src_type = "local" ]] && [[ $storage_type = "azure" ]]; then
sudo az storage blob upload-batch --connection-string "DefaultEndpointsProtocol=https;AccountName=$azure_account_name;AccountKey=$azure_account_key;EndpointSuffix=core.windows.net" --destination $azure_output_container --source $source_output_directory > /dev/null 2>&1
fi


if [[ $src_type = "local" ]] && [[ $storage_type = "azure" ]]; then
sudo az storage blob upload-batch --connection-string "DefaultEndpointsProtocol=https;AccountName=$azure_account_name;AccountKey=$azure_account_key;EndpointSuffix=core.windows.net" --destination $azure_emission_container --source $source_emission_directory > /dev/null 2>&1
fi
