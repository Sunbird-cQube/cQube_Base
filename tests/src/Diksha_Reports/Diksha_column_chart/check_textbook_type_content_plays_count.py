...
class test_textbook_types_data():
    ...
    def test_last30_days(self):
        ...
        coll_type.select_by_index(3)
        self.data.page_loading(self.driver)
        self.driver.find_element_by_id('download').click()
        time.sleep(3)
        self.filename = self.p.get_download_dir() + "/collectionType_textbook_data.csv"
        if not os.path.isfile(self.filename):
            print("Diksha Textbook type of  last 30 days data csv file not downloaded")
        else:
            with open(self.filename) as fin:
                ...
                if int(pc) != int(contentplays):
                    print("Textbook type of last 30 days has difference between screen count value and csv file count ")
                    count = count + 1
            self.data.page_loading(self.driver)
