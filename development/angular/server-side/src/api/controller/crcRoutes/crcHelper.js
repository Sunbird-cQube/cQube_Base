    ...
    try {
        return new Promise((resolve, reject) => {
            ...
            let schoolsVisitedCount1 = {
                totalSchoolsVisited: frequencySchoolDistinct.toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,"),
                totalSchoolsNotVisited: (metadataSchoolDistinct - frequencySchoolDistinct).toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,")
            }
