...
class DikshaTable(unittest.TestCase):
    ...
    def test_textbook(self):
        print("")
        textbook = DikshaTableReport()
        df1 = pd.read_sql_query(textbook.textbook_last_30_days, self.connection)
        df1=df1[['district_id', 'district_name', 'textbook', 'content_name',
               'total_content_plays', 'content_id', 'subject', 'grade', 'medium']]
        df1.sort_values(by=['district_id'], inplace=True)
        pd.set_option("display.max_rows", None, "display.max_columns", None)
        path = config['jsondata']['table_reports_textbook_last_30_days']
        ...
        df2 = df2[['district_id', 'district_name', 'textbook', 'content_name',
               'total_content_plays', 'content_id', 'subject', 'grade', 'medium']]
