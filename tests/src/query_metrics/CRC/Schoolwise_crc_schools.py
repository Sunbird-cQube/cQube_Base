...
class Crc(unittest.TestCase):
    ...
    def test_query(self):
        school = CRC_Query()
        df1 = pd.read_sql_query(school.schoolwise_crc, self.connection)
        ...
        df2 = df2[['schoolname', 'schoolid', 'no_of_schools_per_crc', 'visits_per_school','totalschools']]
        ...
        assert df_diff.empty, "Found difference between s3 bucket metrics and the metrics generated outside cqube for school wise crc schools "
        print("No Difference between s3 bucket metrics and the metrics generated outside cqube for school wise crc schools")
