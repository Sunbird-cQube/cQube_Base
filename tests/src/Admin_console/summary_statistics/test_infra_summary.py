...
class Test_infra_summary(unittest.TestCase):
    ...
    def test_infra_file_details(self):
        self.records = summary_records()
        self.data.page_loading(self.driver)
        x = []
        rows = self.driver.find_elements_by_xpath("//*[@id='table4']/tbody/tr[1]/td")
        for i in range(len(rows)):
            x.append(rows[i].text)
        count = 0
        self.data.page_loading(self.driver)
        y = []
        y.append(self.records.get_infra__file())
        y.append(self.records.get_infra_totalrecords())
        y.append(self.records.get_infra_blank_records())
        y.append(self.records.get_infra_duplicate_records())
        y.append(self.records.get_infra_school_id())
