    ...
    try {
        ...
        for (let i = 0; i < schoolsAttendanceData.length; i++) {
            var obj = {
                ...
                number_of_teachers: Math.round(schoolsAttendanceData[i]['students_count'] / 70).toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,")
            }
            schoolData.push(obj);
        }
        logger.info('--- Attendance school wise api response sent ---');
        res.status(200).send({ schoolData: schoolData, teacherCount: Math.round(jsonData.allSchoolsFooter.students / 70), schoolCount: jsonData.allSchoolsFooter.schools });
    } catch (e) {
...
router.post('/schoolPerCluster', auth.authController, async (req, res) => {
    try {
        ...
        res.status(200).send({ schoolsDetails: schoolsDetails, teacherCount: Math.round(jsonData.footer[clusterId].students / 70), schoolCount: jsonData.footer[clusterId].schools });
    } catch (e) {
        logger.error(`Error :: ${e}`)
        res.status(500).json({ errMessage: "Internal error. Please try again!!" });
    }
