...
router.post('/distWise', auth.authController, async function (req, res) {
    try {
        ...
        if (management != 'overall' && category != 'overall') {
            if (timePeriod != null) {
                fileName = `teacher_attendance/${timePeriod}/school_management_category/${management}/${category}/district.json`;
            } else {
                fileName = `teacher_attendance/school_management_category/${management}/${category}/district_${year}_${month}.json`;
            }
        } else if (management == 'overall' && category != 'overall') {
            if (timePeriod != null) {
                fileName = `teacher_attendance/${timePeriod}/school_management_category/overall_management/${category}/district.json`;
            } else {
                fileName = `teacher_attendance/school_management_category/${year}/${month}/overall_management${category}/district.json`;
            }
        } else if (management != 'overall' && category == 'overall') {
            if (timePeriod != null) {
                fileName = `teacher_attendance/${timePeriod}/school_management_category/overall_category/${management}/district.json`;
            } else {
                fileName = `teacher_attendance/school_management_category/${year}/${month}/overall_category/${management}/district.json`;
            }
        } else {
            if (timePeriod != null) {
        ...
        for (let i = 0; i < districtAttendanceData.length; i++) {
            var obj = {
                ...
                number_of_teachers: districtAttendanceData[i]['teachers_count'].toString().replace(/(\d)(?=(\d\d)+\d$)/g, "$1,"),
