...
if [[ $src_type = "azure" ]] && [[ $storage_type = "azure" ]]; then
	az storage blob copy start-batch --account-key $azure_account_key --account-name $azure_account_name --destination-container $azure_output_container --source-account-key $azure_account_key --source-account-name $azure_account_name --source-container $source_output_container > /dev/null 2>&1
fi

if [[ $src_type = "azure" ]] && [[ $storage_type = "azure" ]]; then
    az storage blob copy start-batch --account-key $azure_account_key --account-name $azure_account_name --destination-container $azure_input_container --source-account-key $azure_account_key --source-account-name $azure_account_name --source-container $source_input_container > /dev/null 2>&1
fi

if [[ $src_type = "azure" ]] && [[ $storage_type = "azure" ]]; then
    az storage blob copy start-batch --account-key $azure_account_key --account-name $azure_account_name --destination-container $azure_emission_container --source-account-key $azure_account_key --source-account-name $azure_account_name --source-container $source_emission_container > /dev/null 2>&1
fi
