...
router.post('/schoolWise', async (req, res) => {
    try {
        logger.info('---PAT heat map cluster wise api ---');
        let { year, grade, subject, examDate, districtId, blockId, clusterId } = req.body
        let fileName = `pat/heatChart/${year}/clusters/${clusterId}.json`
        ...
        schoolDetails = schoolDetails.reduce((unique, o) => {
            if (!unique.some(obj => obj.schoolId === o.schoolId)) {
                unique.push(o);
            }
            return unique;
        }, []);
        ...
        res.status(200).send({ schoolDetails, data });
    } catch (e) {
