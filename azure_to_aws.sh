...
azcopy copy https://$azure_account_name.blob.core.windows.net/$azure_input_container/*$storage_account_sas_token /home/$system_user_name/azure_input --recursive
fi

if [[ $src_type = "azure" ]] && [[ $storage_type = "s3" ]]; then
azcopy copy https://$azure_account_name.blob.core.windows.net/$azure_output_container/*$storage_account_sas_token /home/$system_user_name/azure_output --recursive
fi

if [[ $src_type = "azure" ]] && [[ $storage_type = "s3" ]]; then
azcopy copy https://$azure_account_name.blob.core.windows.net/$azure_emissionput_container/*$storage_account_sas_token /home/$system_user_name/azure_emission --recursive
fi


if [[ $src_type = "azure" ]] && [[ $storage_type = "s3" ]]; then
irbucket=$(aws s3 sync /home/$system_user_name/azure_output s3://$output_bucket)
fi
if [[ $src_type = "azure" ]] && [[ $storage_type = "s3" ]]; then
irbucket=$(aws s3 sync /home/$system_user_name/azure_input s3://$input_bucket)
fi
if [[ $src_type = "azure" ]] && [[ $storage_type = "s3" ]]; then
irbucket=$(aws s3 sync /home/$system_user_name/azure_emission s3://$emission_bucket)
fi
